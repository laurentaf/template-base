...
def check_in_range(col: str, params: dict) -> tuple[str, str]:
    ...
    return (
        f"{col} >= {lo} AND {col} <= {hi}",
        f"SELECT * FROM {{table}} WHERE {col} < {lo} OR {col} > {hi}",
    )
...
def check_matches_regex(col: str, params: dict) -> tuple[str, str]:
    ...
    return (
        f"regexp_matches({col}, '{pattern}')",
        f"SELECT * FROM {{table}} WHERE NOT regexp_matches({col}, '{pattern}')",
    )
...
class DataQualityValidator:
    ...
    def check_table(
        self, table: str, checks: list[QualityCheck], sample_limit: int = 5
    ) -> list[QualityResult]:
        results = []
        for check in checks:
            try:
                ...
                if not rule_fn:
                    results.append(
                        QualityResult(
                            check=check, passed=False, error=f"Unknown rule: {check.rule}"
                        )
                    )
                    continue
                ...
                if failed == 0:
                    dupes = self.con.execute(failure_query.replace("{table}", table)).fetchdf()
                    failed = len(dupes)
                sample = []
                if failed > 0 and sample_limit > 0:
                    sample = (
                        self.con.execute(
                            failure_query.replace("{table}", table) + f" LIMIT {sample_limit}"
                        )
                        .fetchdf()
                ...
                results.append(
                    QualityResult(
                        check=check, passed=failed == 0, failed_rows=failed, sample=sample
                    )
                )
    ...
    def check_all(
        self, table: str, schema: dict[str, list[QualityCheck]]
